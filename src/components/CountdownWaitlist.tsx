...
import { Loader2, Clock, Shield, ArrowRight, Copy, Check, Phone } from 'lucide-react';
import {
  RecaptchaVerifier,
  signInWithPhoneNumber,
  ConfirmationResult,
} from 'firebase/auth';
import { collection, addDoc, query, where, getDocs, serverTimestamp, doc, getDoc, updateDoc, increment, setDoc } from 'firebase/firestore';
...
const generateReferralCode = () =>
  'NAK-' + Math.random().toString(36).substring(2, 8).toUpperCase();
...
// Step of the form: 'form' → 'otp' → 'done'
...
const CountdownWaitlist = () => {
  ...
  // Cache form data between steps
  const formDataCache = useRef<Record<string, string | string[]>>({});
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        ...
        if (statsSnap.exists()) {
          setDisplayCount(statsSnap.data().count + 47);
        } else {
          // If doc doesn't exists, it might be the first time
          setDisplayCount(47);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Cache the form data so we can write to Firestore after OTP verify
    formDataCache.current = {
      ...
      hear_about: data.get('hearAbout') as string || '',
      referral_code_used: data.get('referral') as string || '',
    };
  ...
  // -------------------------------------------------------
  // Step 2: Verify OTP → write to Firestore
  // -------------------------------------------------------
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // OTP is valid and user is now authenticated.
      // Now we can safely check for duplicates since Firestore rules allow read if request.auth != null
      const formattedPhone = formDataCache.current.whatsapp_number as string;
      const q = query(collection(db, 'waitlist'), where('whatsapp_number', '==', formattedPhone));
      const snap = await getDocs(q);
      
      if (!snap.empty) {
        const existing = snap.docs[0].data();
        setIsDuplicate(true);
        setSubmitError("You're already on the waitlist! Your referral code is " + existing.my_referral_code);
        
        // Show the success screen with their existing code
        setRefCode(existing.my_referral_code);
        setRefLink(`${window.location.origin}/?ref=${existing.my_referral_code}`);
        setStep('done');
        return;
      }
      ...
      try {
        ...
        console.error("Error updating public stats:", statsErr);
      }
      ...
      <section className="relative py-[60px] md:py-[100px] overflow-hidden" id="waitlist" style={{ background: 'hsl(var(--secondary))' }}>
        ...
        <div className="max-w-[600px] mx-auto px-5 md:px-6 relative z-10">
          ...
          {step === 'done' ? (
            ...
            >
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-5" style={{ background: 'rgba(184, 134, 11, 0.1)', border: '1px solid rgba(184, 134, 11, 0.2)' }}>
                <Phone size={24} className="text-primary" />
              </div>
              <h3 className="font-heading text-[24px] md:text-[32px] font-light text-foreground mb-2">Verify your number</h3>
              <p className="font-body text-[13px] text-muted-foreground mb-8">
                We sent a 6-digit OTP to <span className="text-foreground font-medium">{formDataCache.current.whatsapp_number as string}</span>
              </p>
              ...
              >
                <div className="mb-5">
                  <label className="font-body text-[12px] font-medium text-foreground/80 mb-2 block">Enter OTP <span className="text-primary">*</span></label>
                  <input
                    type="text"
                    inputMode="numeric"
                    maxLength={6}
                    ...
                    className={inputCls + ' text-center tracking-[0.4em] text-[20px]'}
                    style={{ borderColor: errors.otp ? 'hsl(var(--destructive))' : 'hsl(var(--border))', borderRadius: 8, background: 'hsl(var(--secondary))' }}
                    disabled={loading}
                    autoFocus
                  />
                  {errors.otp && <p className="font-body text-[11px] mt-1.5 text-destructive">{errors.otp}</p>}
                </div>

                <button
                  onClick={handleVerifyOtp}
                  disabled={loading}
                  className="w-full h-[54px] md:h-[58px] font-body text-[15px] md:text-[16px] font-semibold bg-primary text-primary-foreground hover:brightness-110 active:scale-[0.98] transition-all duration-200 disabled:opacity-60 flex items-center justify-center gap-2.5"
                  style={{ borderRadius: 8, boxShadow: 'var(--shadow-gold-glow)' }}
                >
                  {loading ? <><Loader2 className="animate-spin" size={18} /> Verifying...</> : <>Verify & Join <ArrowRight size={16} /></>}
                </button>

                {submitError && <p className="font-body text-[12px] text-center text-destructive mt-3">{submitError}</p>}

                <button
                  onClick={() => { setStep('form'); setOtp(''); setSubmitError(null); }}
                  className="font-body text-[12px] text-muted-foreground hover:text-foreground mt-4 block mx-auto transition-colors"
                >
                  ← Change number
                </button>
              </div>
